...
粉絲功能:留言牆(TO ZAHA)、最愛投票、朝聖護照打卡、數位明信片(1280×800 橫式 PNG 下載)、深度連結 `?work=N`(1-based)、隱私權與 Cookie 說明(首次到訪 banner + 常駐按鈕)、bio 面板底部的低調署名(Nicky Hsieh)。
...
8. **Web Share API 桌機上不能指定分享到特定社群平台**:2026-07-10 已把「分享到社群」整個功能移除(原本按鈕 `#p-share`、`buildPostcardCanvas` 的 IG 限動直式圖模式、`shareBase()` 都刪了)。原因:`navigator.share()` 在桌機(macOS Safari/Chrome)呼叫的是作業系統層級分享選單(Mail/訊息/AirDrop/加入照片…),JS 無法指定跳出 Twitter/Threads/Meta 的分享框;沒有 `navigator.share` 時退回下載 PNG,Safari 預設「下載後自動開啟」又會被誤認為「點分享卻打開相簿」。若之後想重做,考慮改成分別串接各平台官方的網頁分享意圖連結(如 Twitter/X 的 `intent/tweet`),而不是依賴 Web Share API 在桌機達到「分享到指定平台」的效果。
9. **隱私權/Cookie 說明是如實揭露,不是抄 ZHA 官網那套 CookieYes 分類介面**:2026-07-10 參考過 ZHA 官網(zha.com)的 Cookie 同意彈窗(Necessary/Functional/Analytics/Performance/Advertisement 五分類 + Accept All/Reject All),但本站架構完全不同——沒有 Google Analytics、沒有廣告、沒有第一方 Cookie,硬做假的分類開關反而不誠實。改成單純的資訊揭露(`#privacy` modal,重用 `.wall` 的開合機制):本站不設 cookie、localStorage 存哪些 key 做什麼用、留言/投票會公開存進 Google 試算表、第三方(Google Fonts / GAS / Wikimedia)載入時會送出 IP。首次到訪 banner(`#cookie-banner`)只在 `localStorage.zha-notice-ack !== '1'` 時顯示。**排版時 banner 的 `bottom` 值務必大於左下角 `#wall-toggle`+`#privacy-toggle` 兩顆堆疊按鈕的疊加高度**(目前約 180px),不然 banner 會直接蓋住那兩顆按鈕,點不到。
